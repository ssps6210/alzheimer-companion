...
**不需要申请任何 API 密钥。** 默认用跑在你自己电脑上的模型（Ollama），对话文字一个字都不出门。想换成更聪明的云端模型也可以，见下方〈语言与大脑〉。
...
**"会有人偷听我们的对话吗？"**
对话记录存在你的电脑里，不会上传。而且**默认连「文字」都不出门**——回话的 AI 就跑在你自己那台电脑上。

如果你选择改用云端的 AI（回话更聪明一点），那么只有**对话的文字**（不是声音）会送出去，就像你在搜索框打字那样。要不要这样换，决定权在你。
...
**默认跑在你自己的电脑上，不需要任何密钥。** 装 [Ollama](https://ollama.com)（免费）之后
`ollama pull qwen2.5:3b`，这样连对话的文字都不会离开这台机器。
...
两个都不想装也没关系：本机模型没在跑、又填了云端密钥时，它会**自动改用云端并在启动窗口说明**。
想「只用本机、绝不连云端」，把 `conf.yaml` 的 `fallback_api_key_env` 留空即可。

> ⚠️ 显卡只有 8GB 的话要留意：语音识别和声音克隆已经吃掉大部分显存，
> 本机大脑建议用小模型（3B）或让 Ollama 跑 CPU，不然三边抢显存会崩。
...
**这是陪伴工具，不是医疗器械，也不是紧急救援系统。**

「说到不适就通知家人」是尽力而为的提醒，断网、关机、没说话、听错都会失效。
**紧急状况请拨打 120**，不要因为装了这套就减少原本的照护安排。

克隆声音前必须取得本人同意；严禁用于诈骗、冒充或任何违法用途。

完整条款见 **[免责声明与使用规范](DISCLAIMER.zh-CN.md)**——安装或使用即表示你已阅读并同意。
